...
def highlight_danger_actors(world, debug, actor_ids):
...
def carla_params_verification(config):

    logging.basicConfig(format="%(levelname)s: %(message)s", level=logging.INFO)

    # config = config
    # config = config
    # config = config[config["traffic_behavior"]]
    ...
    try:
        ...
        test_tm = test_tm_params(world, tm)

        tm.set_global_distance_to_leading_vehicle(
            config["global_distance_to_leading_vehicle"]
        )
        tm.global_percentage_speed_difference(
            config["global_percentage_speed_difference"]
        )
        ...
        if config["number_of_vehicles"] < number_of_spawn_points:
            random.shuffle(spawn_points)
        elif config["number_of_vehicles"] > number_of_spawn_points:
            msg = "requested %d vehicles, but could only find %d spawn points"
            logging.warning(msg, config["number_of_vehicles"], number_of_spawn_points)
            config["number_of_vehicles"] = number_of_spawn_points
        ...
        for n, transform in enumerate(spawn_points):
            if n >= config["number_of_vehicles"]:
                break
        ...
        number_of_danger_vehicles = int(
            ((config["percentageTrafficBreakingLight"]) / 100)
            * config["number_of_vehicles"]
        )
        danger_vehicles = np.random.choice(
            config["number_of_vehicles"], number_of_danger_vehicles, replace=False
        )
        for i in range(number_of_danger_vehicles):
            print(
                "Setting jumping light percentage to ",
                float(config["ignore_lights_percentage"]),
                " for vehicle: ",
        ...
        if not config["sync"] or not synchronous_master:
            world.wait_for_tick()
        else:
            world.tick()
        ...
        hud = HUD(1080, 720)
        camera_manager = CameraManager(hud, 2.2, world)
        ...
        while True:
            ...
            # tfl_violators, lvd_pairs = test_tm.update_observed_params()
            ...
            # if(world_snapshot.timestamp.elapsed_seconds > 30):
            #     break
            # ticks+=1
            if config["sync"] and synchronous_master:
                world.tick()
            else:
                world.wait_for_tick()
