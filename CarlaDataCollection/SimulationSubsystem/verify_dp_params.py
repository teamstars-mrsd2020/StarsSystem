...
class verify_dp_params:
    def __init__(self, carla_world, traffic_manager, tf_id_list):

        self.world = carla_world
        self.tm = traffic_manager
        self.map = self.world.get_map()
        self.tf_id_list = tf_id_list
        self.tfl_list = [self.world.get_actor(tf_id) for tf_id in self.tf_id_list]
        ...
        self.proximity_vehicle_threshold = 20.0
        self.vehicles_near_junction = []
        self.observed_least_lvd = 1000
    ...
    def update_percentage_running_light(self):
        ...
        for v in vehicle_list:
            ...
            if v.is_at_traffic_light():
                ...
                if traffic_light.id not in self.tf_id_list:
                    continue
                # print(v.id, "is at traffic light: ", traffic_light.id)
        ...
        print("Number of vehicles in stopped set: ", len(self.stopped_vehicles_set))
        print(
            "Number of vehicles in transistion set: ",
            len(self.vehicles_in_transition_set),
        )
    ...
    def _is_vehicle_hazard(self, ego_vehicle, vehicle_list):
        ...
        for target_vehicle in vehicle_list:
            ...
            if is_lead:
                if dist < self.observed_least_lvd:
                    self.observed_least_lvd = dist
    ...
    def vehicle_near_tf_light(self, v):
        def dist(tfl, v):
            return tfl.get_location().distance(v.get_location())

        for tfl in self.tfl_list:
            if dist(tfl, v) < 50:
    ...
    def get_vehicles_near_junction(self):
        ...
        for v in self.vehicle_list:
            ...
            #     traffic_light = v.get_traffic_light()
            #     # print(traffic_light.id)
            #     if(traffic_light.id in self.tf_id_list):
            #         self.vehicles_near_junction.append(v)
            #     else:
            #         continue
            self.vehicle_near_tf_light(v)

    def update_lvd(self):
        def dist(v, ego_v):
            return v.get_location().distance(ego_v.get_location())

        # self.vehicles_near_junction = self.vehicle_list
        self.get_vehicles_near_junction()
        print("Vehicles near junction: ", len(self.vehicles_near_junction))
