...
class BasicSynchronousClient(object):
    ...
    def choose_traffic_light(self, fixed_traffic_light=None):
        ...
        )  # 56,52,58,57,51, 71,62 # orig view - 61
        # 54 good, can rotate sideways also
        # 53-ok, but road blocked
        # NOT 44,43
        # always crashing - black screen = 55
        # graphics issue 75
        return traffic_light, rel_transform

    def setup_camera(self, fixed_traffic_light=None):
        ...
        traffic_light, rel_transform = self.choose_traffic_light(fixed_traffic_light)
    ...
    def spawn_vehicles_walkers(self, safe=False):
        ...
        for actor in self.world.get_actors():
            if "vehicle." in actor.type_id:
                ...
                self.tm.ignore_lights_percentage(actor,80)
        self.tm.set_global_distance_to_leading_vehicle(7)
    ...
    def setup_videowriter(self, FPS):
        ...
        self.videowriter = cv2.VideoWriter(
            output_file_name, fourcc, FPS, (VIEW_WIDTH, VIEW_HEIGHT)
        )
    ...
    def init_client(
        ...
        if(spawn_actors):
            ...
            sleep(5)
        self.setup_camera(fixed_traffic_light)
        self.fixed_traffic_light = fixed_traffic_light
        self.metadata_all = []
    ...
    def de_init_client(self):
        ...
        gt_params_file = os.path.join(
            self.output_folder+"/params", "cgt_" + str(self.intersection_id).zfill(3) + "_" + str(self.side_id).zfill(3) + "_" + str(self.round_).zfill(3) + ".json" #Saves
        )
    ...
    def game_loop(self, number_of_frames=None, num_camera = 1):
        ...
        try:
            ...
            self.verify_dp = verify_dp_params(self.world, self.tm, [50,51,52,53])
            # self.client = carla.Client(hoststr, port)
            # self.client.set_timeout(5.0)
            # self.world = self.client.get_world()
            ...
            if SAVE_DATA:
                ...
                config_file = os.path.join(
                    self.output_folder+"/metadata", "capture_data" + "_c_" + str(self.intersection_id).zfill(3) + "_" + str(self.side_id).zfill(3) +".json" #Saves
                )
                with open(config_file, "w") as f:
                    json.dump(
                        {
                            "sensor_world_matrix": sensor_world_matrix.tolist(),
            ...
            output_folder = self.output_folder+"/metadata/"+"c_" + str(self.intersection_id).zfill(3) + "_" + str(self.side_id).zfill(3) + "_" + str(self.round_).zfill(3)
            if not os.path.exists(output_folder):
                os.makedirs(output_folder)

            while loop_cond(k):
                self.verify_dp.update_observed_params()
                print("TLV percentage: ", self.verify_dp.observed_percentage_jumping_light)
                ...
                final_metadata = ClientSideBoundingBoxes.draw_2d_bounding_boxes(
                    self.display, bounding_boxes, metadata, self, cam_loc_obj,
                )
                if DEBUG_DRAW:
                ...
                if SAVE_DATA:
                    if SAVE_FRAME_WISE_DATA:
                        filename = os.path.join(
                            output_folder, str(frameid) + "_" + str(timestamp) #Framewise - image, final meta data
                        )
    ...
    def game_loop2(self, number_of_frames=None, num_camera = 2):
        """
        Main program loop.
        """
        # For each iteration, save the image and bounding boxes(xmin,ymin,xmax,ymax,classname)
        ...
        try:
            ...
            self.verify_dp = verify_dp_params(self.world, self.tm, [59,60,61,62])
            # self.client = carla.Client(hoststr, port)
            # self.client.set_timeout(5.0)
            # self.world = self.client.get_world()
...
def multiple_rounds_main(tl_id, intersection_id, side_id, round_):
    if local:
        hoststr = "127.0.0.1"
        port = 2000
        tm_port = 8000
    ...
    try:
        ...
        if FIXED_TRAFFIC_LIGHTS:
            number_of_rounds = len(fixed_traffic_lights) * rounds_per_light
            traffic_lights = fixed_traffic_lights * number_of_rounds
        for sc_id in range(number_of_rounds):
            if FIXED_TRAFFIC_LIGHTS:
                traffic_light = traffic_lights[sc_id]
                round_name = "demo_tfl_" + str(traffic_light)
            ...
            client.init_client(
                hoststr, port, tm_port, round_name, output_folder, intersection_id, side_id, round_, FPS, safe, traffic_light, spawn_actors=SPAWN_ACTORS
            )
...
if __name__ == "__main__":
    ...
    side_id = 0
    round_ = 1
    multiple_rounds_main(tl_id, intersection_id, side_id, round_)
